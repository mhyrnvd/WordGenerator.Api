using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;
using WordGenerator.Api.Domain.Entities;

namespace WordGenerator.Api.Infra.Configurations
{
    public class AttachmentSectionConfiguration : IEntityTypeConfiguration<AttachmentSection>
    {
        public void Configure(EntityTypeBuilder<AttachmentSection> builder)
        {
            builder.ToTable("AttachmentSections");
            builder.HasKey(x => x.Id);

            builder.Property(x => x.Title)
                .HasMaxLength(200)
                .HasDefaultValue("الف) پیوست‌ها");

            builder.Property(x => x.IsActive)
                .HasDefaultValue(true);

            builder.Property(x => x.Order)
                .HasDefaultValue(1);

            // رابطه با DocumentTemplate - Restrict
            builder.HasOne(x => x.DocumentTemplate)
                .WithOne(x => x.AttachmentSection)
                .HasForeignKey<AttachmentSection>(x => x.DocumentTemplateId)
                .OnDelete(DeleteBehavior.Restrict);

            // رابطه با Elements - Restrict
            builder.HasMany(x => x.Elements)
                .WithOne(x => x.AttachmentSection)
                .HasForeignKey(x => x.AttachmentSectionId)
                .OnDelete(DeleteBehavior.Restrict);
        }
    }
}